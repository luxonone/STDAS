...
impl BootstrapAdmin {
    pub(crate) fn from_env() -> Result<Self, BootstrapAdminError> {
        Self::from_lookup(|key| env::var(key).ok(), None)
    }

    pub(crate) fn from_env_with_password(password: String) -> Result<Self, BootstrapAdminError> {
        Self::from_lookup(|key| env::var(key).ok(), Some(password))
    }
    ...
    fn from_lookup(
        ...
        let password = password_override
            .or_else(|| lookup(BOOTSTRAP_ADMIN_PASSWORD_ENV))
...
mod tests {
    ...
    fn bootstrap_admin_uses_mes_aligned_defaults_and_requires_password() {
        let admin = BootstrapAdmin::from_lookup(
            |key| match key {
        ...
        let missing = BootstrapAdmin::from_lookup(|_| None, None);
    ...
    #[test]
    fn bootstrap_admin_accepts_interactive_password_without_storing_local_file() {
        let admin = BootstrapAdmin::from_env_with_password("admin@123".to_owned())
            .expect("interactive password should create bootstrap admin");

        assert_eq!(admin.password, "admin@123");
    }

    #[test]
    fn bootstrap_admin_rejects_empty_interactive_password() {
        let missing = BootstrapAdmin::from_env_with_password("   ".to_owned());
