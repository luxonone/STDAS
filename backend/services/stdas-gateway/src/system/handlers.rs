...
use super::dto::{HealthResponse, PreflightResponse};

pub(super) fn health() -> impl IntoResponse {
    let status = super::service::health();
    Json(ApiResponse::success(HealthResponse::from(status)))
}

pub(super) fn preflight() -> impl IntoResponse {
    let preflight = super::service::preflight();
...
mod tests {
    ...
    async fn health_endpoint_returns_success_envelope() -> Result<(), Box<dyn std::error::Error>> {
        let response = super::health().into_response();
    ...
    async fn preflight_endpoint_returns_contract_data() -> Result<(), Box<dyn std::error::Error>> {
        let response = super::preflight().into_response();
