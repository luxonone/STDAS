...
本 ADR 的口径不是“从 Loco 最小迁移”，而是把当前最小应用作为新起点，重新建立 Axum service crate 结构。项目结构参考 Melrose《Rust + Axum 后端架构设计文档》（https://melrose1994.com/index.php/2026/04/29/rust-axum-%e5%90%8e%e7%ab%af%e6%9e%b6%e6%9e%84%e8%ae%be%e8%ae%a1%e6%96%87%e6%a1%a3/）中展示的目录化分层：`app.rs`、配置、DB/cache 边界、错误、extractor、中间件、模型、DTO、repository、service、handler、route、state 和 telemetry。

## 决策

`stdas-gateway` 直接采用 Axum 作为 HTTP 应用框架。当前 crate path 为 `apps/api`；目录落点决策见 [ADR-0012](0012-backend-api-app-layout.md)。
...
- `src/state.rs` 是 Axum shared state 边界；当前挂载最小 `SystemService`，未来可挂载 service client 或 SQLx pool。
...
- `src/handlers/` 只做 extractor、协议 DTO 和 response mapping。
- `src/dto/` 放 API request/response DTO 和 response envelope。
- `src/models/` 放服务内部模型；后续 domain model 不得被 HTTP envelope 污染。
- `src/services/` 放业务逻辑和 usecase 入口；handler 不直接拼业务流程。
- `src/repositories/` 放数据访问边界；当前没有数据库用例，不引入 unused SQLx dependency。
- `src/errors/` 放 typed error 和 API error mapping。
- `src/telemetry.rs` 放 tracing、metrics、request id 等观测性边界；当前只保留归属边界。
- `src/utils/` 放小型、领域无关的工具函数。
...
其中 SQLx pool、transaction、migration 调用和 repository 实现位于 repository/data access 边界，handler 不直接写 SQL。
...
- `stdas-gateway` 移除 Loco/MVC 风格目录，采用 `main/app/config/server/state/routes/middleware/handlers/dto/models/services/repositories/errors/telemetry/utils` 的 Axum service crate 结构。
