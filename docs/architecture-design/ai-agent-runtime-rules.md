...
4. [AI Agent 运行时规则](ai-agent-runtime-rules.md)
5. 与任务相关的分区 README。
6. 与任务相关的专题文档。
...
- 涉及 Git、GitHub、GitLab、提交、推送、分支、PR/MR、合并、回退、reset、clean、rebase 或 force push → [Git Commit and Collaboration SPEC](../specs/git-commit-collaboration-spec.md) + [Git / GitHub 安全 SOP](git-github-sop.md)
...
| 任务类型 | 必读文档 | 说明 |
|----------|----------|------|
| SPEC 分区、启动上下文、项目铁律 | [SPEC 中心](../specs/README.md)、[AI Agent Startup Context SPEC](../specs/agent-startup-context-spec.md) | SPEC 是项目内最高优先级规则，普通文档只能引用不能覆盖 |
...
| Git remote 绑定、提交、推送、分支、PR/MR、合并、回退 | [Git Commit and Collaboration SPEC](../specs/git-commit-collaboration-spec.md) + [Git / GitHub 安全 SOP](git-github-sop.md) | 版本控制操作必须先检查状态，不直接执行破坏性命令 |
...
| Rust 代码生成、所有权、错误处理、异步和测试 | [Rust Coding Guidelines SPEC](../specs/rust-coding-guidelines-spec.md)、[Rust 代码质量规则](../backend-design/rust-code-quality-rules.md)、[Rust AI 代码生成规则](../backend-design/rust-ai-code-generation-rules.md)、[Rust 高质量项目参考与模式](../backend-design/rust-reference-projects-and-patterns.md) | Rust SPEC 定 coding baseline，后端设计维护具体实现规则 |
...
- 新增或修改铁律时，必须放入 `docs/specs/`；普通规则必须放入对应事实来源文档，不在多个文档中复制规则正文。
- 涉及代码生成、实现路径选择、偏航提醒或待优化标记时，必须读取 [AI 代码生成治理机制](ai-code-generation-governance.md)。
- 生成或修改 Rust 代码前，必须读取 [Rust Coding Guidelines SPEC](../specs/rust-coding-guidelines-spec.md)、[Rust 代码质量规则](../backend-design/rust-code-quality-rules.md) 和 [Rust AI 代码生成规则](../backend-design/rust-ai-code-generation-rules.md)；非平凡后端实现还必须读取 [Rust 高质量项目参考与模式](../backend-design/rust-reference-projects-and-patterns.md)。
...
STDAS 的维护者可能明确说明自己对 Rust、前端或后端工程并不熟悉。AI Agent 必须把这类说明视为风险提示，而不是把用户给出的路径、目录或技术判断直接当成正确方案。

当用户提出移动、删除、替换或重组项目结构时，AI Agent 必须先做专业判断：

1. 区分目标是源码、配置、lockfile、构建产物、本地依赖、临时 scratch、外部参考材料还是 agent 本地状态。
2. 检查 Git 跟踪状态、ignore 规则、现有文档和对应工具链推荐位置。
3. 对 `Cargo.toml`、`Cargo.lock`、`package.json`、`pnpm-lock.yaml`、`node_modules/`、`target/`、`dist/`、配置目录和数据目录给出明确分类后再操作。
4. 如果用户方向会破坏架构边界、数据安全、可恢复性、构建 reproducibility 或版本管理清晰度，必须先指出风险并给出推荐替代方案。
5. 只有在确认内容可再生成、未被 Git 跟踪、路径位于预期工作区内，并且删除不会丢失用户工作后，才允许清理本地生成目录。

AI Agent 的职责是保护项目资产和长期可维护性。用户的猜测可以作为排查线索，但不能替代项目文档、官方实践、源码事实和本地验证结果。
...
1. 本次修改属于哪个目录的事实来源？
2. 是否涉及 SPEC 或需要把普通文档规则升级为 SPEC？
3. 是否需要同步更新前端、后端或架构的引用关系？
4. 是否改变了已确认的架构边界或 ADR？
5. 是否引入了新的 UI/API/数据/权限规则？如果是，是否写入了对应权威文档？
6. 是否存在与现有规则重复或冲突的描述？

## 文档修改规则

- 修改前先读取当前目录的 `README.md`。
- 修改 SPEC 前必须读取 [SPEC 中心](../specs/README.md)，并说明对普通文档引用关系的影响。
