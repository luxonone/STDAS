...
当前本机环境记录见 [Phase 0.5 环境验证记录](backend-design/phase-0-5-environment-validation.md)。Phase 0 / 0.5 基线为 Windows + Scoop 本机工具链，前端统一使用 pnpm；Gateway HTTP 应用层采用 Loco（基于 Axum）；Redis 已安装并可按缓存策略启用；Windows 本地开发不安装、不使用 Docker。
...
3. [Git / GitHub 安全 SOP](architecture-design/git-github-sop.md)
4. [前后端同步设计](architecture-design/frontend-backend-sync-design.md)
5. [首批功能切片 V1](architecture-design/feature-slices-v1.md)
6. [Phase 0.5 环境验证记录](backend-design/phase-0-5-environment-validation.md)
