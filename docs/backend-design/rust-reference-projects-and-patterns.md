...
| [Rust Coding Guidelines 中文站](https://rust-coding-guidelines.github.io/rust-coding-guidelines-zh/) | 命名、格式、注释、数据类型、函数设计、async、Unsafe Rust 等通用编码规范 | 作为 STDAS Rust coding baseline；具体采用方式见 [Rust Coding Guidelines SPEC](../specs/rust-coding-guidelines-spec.md)，不得覆盖 STDAS 架构和 API 契约 |
...
| [axum docs](https://docs.rs/axum/latest/axum/) / [axum examples](https://github.com/tokio-rs/axum/tree/main/examples) | router、extractor、state、handler、middleware、response 和错误映射 | `stdas-gateway` 直接按 Axum 组织 HTTP 层；handler 只做协议适配，业务流程放入 service/usecase，错误通过统一映射返回 API response |
| [Melrose《Rust + Axum 后端架构设计文档》](https://melrose1994.com/index.php/2026/04/29/rust-axum-%e5%90%8e%e7%ab%af%e6%9e%b6%e6%9e%84%e8%ae%be%e8%ae%a1%e6%96%87%e6%a1%a3/) | Axum 后端完整目录化结构示例：`app.rs`、`config/`、`db/`、`cache/`、`errors/`、`extractors/`、`middleware/`、`models/`、`dto/`、`repositories/`、`services/`、`handlers/`、`routes/`、`state.rs`、`telemetry.rs`、`tests/` | `backend/services/stdas-gateway` 采用其中的 API app 内部分层、`AppState` 依赖注入、`Handler -> Service -> Repository` 调用方向、DTO/model 分离和 middleware 边界；当前不照搬 DB pool、Redis、JWT/RBAC、OpenAPI、metrics、object storage、Docker/K8s 或后台任务，直到出现真实功能切片 |
...
1. **HTTP handler 是协议适配器**：接收 extractor、校验协议输入、调用 service/usecase、返回统一响应；不直接承载 SQL、权限、缓存、事件和业务流程。
...
- fat handler：Axum handler 同时做解析、权限、SQL、业务、缓存、事件和响应拼装。
...
- 当前问题最接近哪个参考来源：Tokio async、axum API、SQLx persistence、thiserror/tracing、Meilisearch 搜索/任务，还是 Qdrant 查询/存储/任务？
- 参考来源中可借鉴的具体模式是什么？
