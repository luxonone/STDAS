...
| [Melrose《Rust + Axum 后端架构设计文档》](https://melrose1994.com/index.php/2026/04/29/rust-axum-%e5%90%8e%e7%ab%af%e6%9e%b6%e6%9e%84%e8%ae%be%e8%ae%a1%e6%96%87%e6%a1%a3/) | Axum 后端完整目录化结构示例：`app.rs`、`config/`、`db/`、`cache/`、`errors/`、`extractors/`、`middleware/`、`models/`、`dto/`、`repositories/`、`services/`、`handlers/`、`routes/`、`state.rs`、`telemetry.rs`、`tests/` | `apps/api` 采用其中的 API app 内部分层、`AppState` 依赖注入、`Handler -> Service -> Repository` 调用方向、DTO/model 分离和 middleware 边界；当前不照搬 DB pool、Redis、JWT/RBAC、OpenAPI、metrics、object storage、Docker/K8s 或后台任务，直到出现真实功能切片 |
