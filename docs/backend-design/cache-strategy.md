...
本文件定义 STDAS 缓存策略和 Redis 使用边界。当前 Windows 本机已通过 Scoop 安装 Redis，Redis 可在缓存 adapter、限流、token revocation 或 Options cache 需要时启用；但缓存能力从第一版开始仍必须通过接口抽象，避免业务代码绑定具体缓存实现。
...
| Overview snapshot metadata | 否 | PostgreSQL |
...
overview:{customer_scope_hash}:{customer}:{product}:{test_type}:{test_station}:{time_range}:{profile_version}:{snapshot_version}
...
- Overview 缓存必须包含时间范围、profile version 和 snapshot version。
...
| Phase 4 | 若 Options、Overview、热点分析结果有性能压力，接入 Redis 作为共享缓存 |
...
| Overview 访问量高，PostgreSQL 压力上升 | 需要缓存热点快照 |
