...
| 类型 | 场景 | 执行方式 |
|------|------|----------|
| 在线轻查询 | Overview、Lot Detail、小范围趋势 | gateway 同步调用 `analytics-service` |
| 交互式中查询 | 多 lot、多参数图表 | 同步优先，超预算转 workflow 异步 job |
| 离线重查询 | 大导出、复杂相关性、批量报告 | NATS event + workflow job |

## 分析能力边界

`analytics-service` 不穷举所有分析方法。架构只定义分析执行框架，具体分析能力通过内置模块、配置模板、算法 registry 和客户专属扩展持续演进。
...
面向普通工程用户的查询 API 应优先使用业务语义：`LotNo`、`CustomerLotNo`、`TestType`、`TestStation`、`TestAttempt`、`LotEndTime`。`DataVersion` 和 `DataVersionPolicy` 是内部可复现和追溯语义，不应在 FT Overview、Lot List 等普通页面作为主筛选控件暴露。
- `lot_scope`。
- 查询预算。
- 权限和审计。
- 同步/异步执行策略。
...
`analytics-service` 必须把查询语义和执行后端分离。初始可用 PostgreSQL 聚合表；参数分析规模增长后切换 DuckDB；高并发场景再接 ClickHouse。
