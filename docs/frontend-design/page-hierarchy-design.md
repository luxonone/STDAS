...
- 上下文连续：从总览、数据对象、告警、模板进入分析时，客户、产品、测试类型、测试站点、测试次数、LotEndTime 时间范围和来源上下文不能丢。
...
| 能力 | 规则 |
|------|------|
| 登录和会话 | 未登录进入 `/login`，登录后进入 `/app/overview` |
...
| 页面族 | 职责 | 典型页面 |
|------|------|----------|
| Overview | 判断当前最需要关注的客户、产品、测试类型-测试站点、批次和告警 | FT KPI、近期趋势、risky lots、open alerts、快捷入口 |
...
| 来源 | 目标 | 必须携带的上下文 |
|------|------|------------------|
| Overview | Data Explorer | customer、product、test type、test station、LotEndTime range |
| Overview | Alerts & Investigation | alert filter、LotEndTime range、test type、test station |
