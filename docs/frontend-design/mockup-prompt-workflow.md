...
- LotNo 是工厂内部批次号，不等同于 WaferLot。需要同时支持 CustomerLotNo、WaferLot 等字段，但 Lot List 和普通工程页面默认以 LotNo 为主。
...
- 文件名必须带顺序号，例如 `01-login.png`、`02-data-explorer-lot-list.png`、`03-lot-detail.png`。
...
- 产品分析页面可以提供产品、测试类型、测试站点、LotNo 维度切换，主区域可包含良率、Fail Bin、Retest、Parametric Test 分析 tabs。FT 第一阶段不展示 wafer 维度切换。
...
STDAS 登录后工程页面必须按场景显式呈现以下一等概念。Data Explorer 可以使用 List Snapshot / Query Summary 等页面语义；Analysis Workspace、Evidence、Export 和 Investigation 必须显示 QuerySnapshot 或等价稳定引用：
...
DataProfile、DataVersion、DataVersion policy 可以在治理、诊断、Evidence、Export 元数据中出现，但不应作为 Data Explorer、登录页或普通分析入口的主控件。
...
| 2 | Data Explorer / Lot List | 必须参考 Login，延续产品气质、颜色、字体密度、全局壳层方向、导航、上下文栏、表格密度和状态表达。 |
| 3 | Lot Detail / DataVersion Trace | 必须参考前 2 张，延续 Data Explorer 的表格、抽屉、DataVersion 和 lineage 表达。 |
| 4 | Analysis Workspace | 必须参考前 3 张，延续工具栏、筛选面板、表格、图表和状态表达。 |
| 5 | Alerts & Investigation | 必须参考前 4 张，延续工作区布局、Evidence、QuerySnapshot 和 case 面板表达。 |
| 6 | Jobs & Exports | 必须参考前 5 张，延续任务状态、表格、详情面板和导出状态表达。 |
| 7 | System Governance | 必须参考前 6 张，延续系统级页面的壳层、表格、版本状态、diff 和审计表达。 |
...
- For pages after login, show left navigation with Data Explorer, Analysis Workspace, Alerts & Investigation, Jobs & Exports, System Governance.
...
```text
Avoid generic SaaS admin dashboard, marketing landing page, hero section, decorative illustration, stock photo, dark neon command center, KPI-only dashboard, empty cards, oversized typography, excessive whitespace, mobile layout, CRM/ERP/MES screen, finance dashboard, map dashboard, multi-factory portal, external customer portal, SSO provider login, environment profile selector, factory context selector on login, OSAT tenant selector, visible OneData/Exensio/SAS product names, Wafer Count or Wafer Map as default FT page content, CP wafer analysis on FT pages, DataProfile selector on normal engineering pages, DataVersion Policy selector on normal engineering pages, User Role selector, copied Exensio UI, copied OneData UI, copied SAS UI, fake official logo, unreadable random text, meaningless chart labels, purely aesthetic charts without axes or legends, charts without FT semiconductor context, tables without LotNo/TestType/TestStation/LotEndTime fields, UI without filters, UI without query/list snapshot context, UI without error/partial/stale status.
```
...
- Show query id or list snapshot id.
...
### 12.3 Lot Detail / DataVersion Trace
