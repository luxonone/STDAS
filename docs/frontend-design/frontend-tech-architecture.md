...
| UI Strategy | STDAS-owned semantic workbench components；需要通用 UI primitive 时优先采用 shadcn/ui 这类 copy-in、源码可控模式 |
...
- 默认优先建设 STDAS 自有业务语义组件，例如 LotGrid、FilterShelf、ContextBar、JobQueueDrawer、EvidencePanel。
- 当确实需要 Button、Dialog、Popover、Select、Tabs、Toast 等通用 primitive 时，优先采用 shadcn/ui 这种复制到项目源码内、可按 STDAS 设计 token 修改的组件模式。
- 不使用重型黑盒 UI 组件库替代 STDAS 视觉设计，不让组件库默认主题覆盖 Pencil/Figma 已确认设计稿。
- 引入通用 primitive 前必须先确认它不会改变现有架构、样式体系、可访问性和测试策略。
