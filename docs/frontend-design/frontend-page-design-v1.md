...
本文定义 STDAS 第一阶段前端页面设计。它在 [页面层级设计](page-hierarchy-design.md)、[前端工作台设计](workbench-design.md)、[UI/UX 约束](ui-ux-constraints.md) 和 [首批功能切片 V1](../architecture-design/feature-slices-v1.md) 之上，补齐所有一级页面、次级页面、独立页面和页面级设计契约。
...
第一阶段面向 FT 测试系统，BI、BIT、SLT 等成品形态相关测试按客户/产品流程纳入 TestType / TestStation 语义；CP 晶圆测试系统暂不进入默认页面。WaferLot / WaferNo / X / Y 仅在 FT 数据实际提供且分析场景需要时作为可选分析字段出现，不作为 Overview、Lot List 或全局上下文主维度。
...
- Overview 不是管理层大屏，而是 FT 开班检查和异常入口。
- Data Explorer 不是 CRUD 表，而是 Lot / Run / File / DataVersion 的高密度对象浏览和多 Lot 选择入口。
...
| `04 Screens` | Login、Overview、Data Explorer、Lot Detail、Analysis Workspace、Alerts、Jobs、Governance 和独立页面 |
...
| API | `POST /api/v1/auth/login`、`POST /api/v1/auth/refresh`、`GET /api/v1/auth/me` |
...
|----|------|
| Route | 登录后所有 `/app/*` |
| 主导航 | Overview、Data Explorer、Analysis Workspace、Alerts & Investigation、Jobs & Exports、System Governance |
...
|----|------|
| Route | `/app/overview` |
| 用户目标 | 开班或日常巡检时快速判断 FT 数据风险和下一步入口 |
| 页面结构 | Context/filter bar + compact KPI strip + yield trend + fail bin trend/Pareto + risky lots table + alerts side panel |
| 主筛选 | customer、product、TestType、TestStation、TestAttempt、LotEndTime range |
| KPI | Lot count、Avg Yield、Retest Rate、Open Alerts、Running Jobs、Data Freshness；不展示 Wafer Count |
| 表格字段 | LotNo、CustomerLotNo、Product、TestType、TestStation、TestAttempt、Yield、Top Fail Bin、Tester、Handler、Test Program、LotEndTime |
| 图表 | FT Yield Trend、Fail Bin Pareto/Trend、Retest Funnel、小型 Data Quality strip |
| 状态 | Overview Snapshot、Snapshot Time、stale、partial、cached、permission denied |
| 跳转 | Risky Lot -> Lot Detail；Top Bin -> Analysis Workspace；Alert -> Alert Detail；KPI card -> Data Explorer |
| API | `GET /api/v1/overview/summary`、options APIs |
| 禁止 | 管理层大屏、KPI-only、DataProfile selector、DataVersion policy selector、Updated 主时间列 |
...
| Overview | customer、product、test_type、test_station、test_attempt、lot_end_time_range | 局部展开、局部排序 | 临时权限、未授权对象 |
...
2. Overview：建立 AppShell、ContextBar、KPI/table/chart/status 语言。
3. Data Explorer / Lot List：固化 dense grid、filter、selection、drawer 模式。
4. Lot Detail / DataVersion Trace：固化 detail tabs、lineage、version/status 模式。
5. Analysis Workspace：固化核心工作区、图表/表格联动、Evidence 模式。
6. Alerts & Investigation：固化 Case、Evidence、Rule Trigger Context。
7. Jobs & Exports：固化 job lifecycle、export wizard、failure diagnostics。
8. System Governance：固化 version diff、impact analysis、publish/rollback/audit。
9. Independent Pages：固化 permission、expired、maintenance、viewport fallback。
...
| F1 | Overview、Lot List、Lot Detail 基础 | 打通主查询和主对象浏览 |
