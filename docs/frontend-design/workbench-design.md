...
- 快速筛选、对比、下钻和导出。
- 图表与表格并列组织，避免长页面堆叠。
- 支持从 Data Explorer、Alerts、Template 快速进入分析上下文。
...
当前第一阶段先开发 FT 测试系统。CP 晶圆测试系统暂不考虑；Wafer 相关字段只在 FT 数据实际提供且具体分析需要时作为可选字段出现，不作为 Lot List 或全局上下文的默认主维度。
...
- Engineering Workbench：Data Explorer、Analysis Workspace、Alerts & Investigation、Jobs & Exports。
