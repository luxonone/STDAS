...
- [Keep a Changelog 1.1.0](https://keepachangelog.com/en/1.1.0/)：changelog 面向人，使用 `[Unreleased]` 和 `Added` / `Changed` / `Deprecated` / `Removed` / `Fixed` / `Security` 分类。
...
1. 面向用户、维护者或集成方有长期追踪价值的变化必须更新 `CHANGELOG.md` 的 `[Unreleased]`。
2. Changelog 不记录每个 commit 的流水账。
3. 使用 `Added`、`Changed`、`Deprecated`、`Removed`、`Fixed`、`Security` 分类。
4. 纯内部重排、临时验证、无对外影响的小测试变更可以不写 changelog。
5. 修改 Git 流程、SPEC、架构规则、API 契约、用户可见页面、数据语义、安全策略时必须写 changelog。
